...
var cheerioHtml = function(htmlcontent) {
    return cheerio.load(htmlcontent);
};

var getUrl = function (url){
    rest.get(url).on('complete', function(result){
        var check = checkHtmlFile(cheerioHtml(result), program.checks);
        console.log(JSON.stringify(check, null, 4));
    });
};
...
if(require.main == module) {
    ...
    if (program.file){
        var file = getHtmlFile(program.file);
